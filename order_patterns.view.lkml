...
FROM   (SELECT oars_and_alps.email                          AS user_id,
        ...
        FROM   oars_and_alps
                WHERE {% condition lineitem_sku %} oars_and_alps.lineitem_sku {% endcondition %}
        GROUP  BY 1,2,3,5)
 ;;
  }

  filter: lineitem_sku {
    suggest_dimension: lineitem_sku
  }
  ...
  dimension: lifetime_orders_tier {
    ...
    style: integer
    drill_fields: [category,ordered_hour_of_day,lineitem_sku]
  }
